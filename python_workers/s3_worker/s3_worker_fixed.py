...
logger = logging.getLogger("s3_worker_fixed")
...
def get_devices_from_redis():
    ...
    with tracer.start_as_current_span("get_devices_from_redis"):
        try:
            ...
            logger.info(f"Found {len(device_keys)} devices in Redis")
            ...
            for key in device_keys:
                ...
                if device_data:
                    try:
                        ...
                        # Dodaj diagnostykę dla pierwszego urządzenia
                        if len(devices) == 1:
                            logger.info(f"Sample device data JSON structure: {list(device_json.keys())}")
...
def get_file_content(file_path):
    ...
    with tracer.start_as_current_span("get_file_content"):
        try:
            logger.info(f"Fetching file content from: {file_path}")
            response = s3_client.get_object(Bucket=config['S3_BUCKET'], Key=file_path)
            content = response['Body'].read().decode('utf-8')
            try:
                data = json.loads(content)
                # Log a small preview of the data
                data_preview = str(data)[:100] + '...' if len(str(data)) > 100 else str(data)
                logger.info(f"Successfully parsed JSON from {file_path}: {data_preview}")
                return data
    ...
    with tracer.start_as_current_span("process_device_files"):
        ...
        for hostname, device_data in devices.items():
            ...
            if backup_path in all_files_set:
                logger.info(f"Found backup.json for {hostname} at {backup_path}")
                backup_data = get_file_content(backup_path)
                if backup_data:
                    # Używamy nazwy klucza 'backup_data' dla zgodności z poprzednią wersją skryptu
                    updated_data['backup_data'] = backup_data
                    ...
                    updated_data['backup_data'] = {}
                files_processed += 1
            else:
                logger.warning(f"No backup.json found for {hostname} at {backup_path}")
                updated_data['backup_data'] = {}
                missing_files.append('backup.json')
            
            # Check config_validation.json
            if config_validation_path in all_files_set:
                validation_data = get_file_content(config_validation_path)
                updated_data['validation'] = validation_data
                files_processed += 1
            else:
            ...
            if operational_status_path in all_files_set:
                opstatus_data = get_file_content(operational_status_path)
                updated_data['opstatus'] = opstatus_data
                files_processed += 1
            else:
                updated_data['opstatus'] = {}
                missing_files.append('operational_status.json')
            
            # Update Redis
            redis_key = f"device:{hostname}"
            try:
                # Dodajmy szczegółową diagnostykę
                logger.info(f"Redis data before update for {hostname} - keys: {list(updated_data.keys())}")
                if 'backup_data' in updated_data:
                    backup_data_size = len(str(updated_data.get('backup_data', {})))
                    logger.info(f"backup_data dla {hostname} ma rozmiar: {backup_data_size} znaków")
                    # Sprawdźmy czy to dict czy string
                    logger.info(f"backup_data type: {type(updated_data['backup_data']).__name__}")
                
                json_data = json.dumps(updated_data)
                redis_client.set(redis_key, json_data)
                logger.info(f"Updated Redis for {hostname} with new data")
                
                # Weryfikacja po zapisie
                verification = redis_client.get(redis_key)
                if verification:
                    verified_data = json.loads(verification)
                    if 'backup_data' in verified_data:
                        logger.info(f"VERIFICATION OK: backup_data jest w Redis dla {hostname}")
                        logger.info(f"backup_data struktura: {list(verified_data['backup_data'].keys()) if isinstance(verified_data['backup_data'], dict) else 'not a dict'}")
                    else:
                        logger.warning(f"VERIFICATION FAILED: backup_data NIE ISTNIEJE w Redis dla {hostname}!")
                        logger.warning(f"Klucze w Redis dla {hostname}: {list(verified_data.keys())}")
                        logger.warning(f"Dokładny JSON: {verification[:500]}...")
                devices_updated += 1
            except Exception as e:
            ...
            if missing_files:
                ...
                logger.warning(f"Device {hostname} (Vendor: {vendor}, Device_Class: {device_class}) is missing files: {', '.join(missing_files)}")
                logger.warning(f"  - Expected paths:")
                logger.warning(f"    * backup.json: {backup_path}")
                logger.warning(f"    * config_validation.json: {config_validation_path}")
                logger.warning(f"    * operational_status.json: {operational_status_path}")
...
def main():
    ...
    if devices:
        ...
        for hostname in sample_hostnames:
            logger.info(f"Sample device data for {hostname}:")
