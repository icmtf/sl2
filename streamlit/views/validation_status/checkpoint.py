...
def load_validation_data():
    """Load validation data for CheckPoint devices from Redis
    
    Retrieves and processes all CheckPoint device validation data from Redis,
    including device details and validation results.
    
    Returns:
        tuple: (checkpoint_data, checkpoint_details, device_info)
            - checkpoint_data: List of device data for the table display
            - checkpoint_details: Dictionary of validation failures by hostname
    ...
    try:
        ...
        for key in device_keys:
            ...
            if device_data:
                ...
                # Check if device has config_validation data and is CheckPoint
                if "config_validation" in device_json and isinstance(device_json["config_validation"], dict):
                    ...
                    # Check if it's a CheckPoint device
                    if validation_data.get('vendor') != 'CheckPoint':
                        continue
        ...
        return checkpoint_data, checkpoint_details, device_info
    except Exception as e:
        st.error(f"Error loading validation data: {str(e)}")
        return [], {}, {}

def display_device_details(hostname, device_info, checkpoint_details):
    ...
    Args:
        ...
        checkpoint_details (dict): Dictionary with validation failures by hostname
    """
    device = device_info.get(hostname, {})
    failed_checks = checkpoint_details.get(hostname, {})
...
try:
    ...
    # Load and process CheckPoint validation data
    checkpoint_data, checkpoint_details, device_info = load_validation_data()
    ...
    for index, row in edited_df.iterrows():
        if row['Details']:
            ...
            display_device_details(device_id, device_info, checkpoint_details)
