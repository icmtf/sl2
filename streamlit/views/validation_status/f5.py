...
def display_device_details(hostname, device_info, f5_details):
    """Display detailed device information including validation status"""
    device = device_info.get(hostname, {})
    failed_checks = f5_details.get(hostname, {})
    ...
    try:
        ...
        for key in device_keys:
            ...
            if device_data:
                ...
                # Check if device has config_validation data and is F5
                if "config_validation" in device_json and isinstance(device_json["config_validation"], dict):
                    ...
                    # Check if it's a F5 device
                    if validation_data.get('vendor') != 'F5':
                        continue
        ...
        return f5_data, f5_details, device_info
    except Exception as e:
...
try:
    ...
    f5_data, f5_details, device_info = load_validation_data()
    
    if not f5_data:
        st.warning("No F5 devices found in validation data")
        st.stop()
        
    df = pd.DataFrame(f5_data)
    ...
    for index, row in edited_df.iterrows():
        if row['Details']:
            ...
            display_device_details(device_id, device_info, f5_details)
